...
R works best when taking advantage of multiple packages and `rmarkdown` is no exception. For example, `knitr` is a package worth exploring as it allows the `rmarkdown` to be converted to different document types. `rmarkdown` takes advantage of this relationship and knowing more about `knitr` will help make your rendered documents that much better.

`rmarkdown` is not without challenges. The list formatting can be especially tedious, you have to get the spacing just right. The HTML and Word outputs behaved differently and it took a lot of research to figure out what worked best for both or to find a workaround.
...
#### Word Template
`rmarkdown` is great for rendering to an HTML doc, but what about Word? Some of the markup tags work in Word, but most do not. Plus, what about all the great formatting functionality in Word? Well, with a little code in the YAML header we can have our `rmarkdown` rendered to a custom word template with the formatting already setup. The word template used for this project (RMarkdown_Template.docx) has a customized Title (Red), In line Code (light Green) and borders around all the Tables.
